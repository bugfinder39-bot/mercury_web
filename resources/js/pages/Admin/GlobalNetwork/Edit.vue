...
const form = useForm({
    ...
    type: props.location.type || 'Corporate Office',
    latitude: props.location.latitude || 0,
    longitude: props.location.longitude || 0,
    address: props.location.address || '',
    google_maps_url: props.location.google_maps_url || '',
    phone: props.location.phone || '',
    email: props.location.email || '',
    website: props.location.website || '',
    description: props.location.description || '',
    order: props.location.order || 1,
    is_active: props.location.is_active ?? true,
    is_featured: props.location.is_featured ?? false,
    marker_color: props.location.marker_color || '#0284c7',
    marker_icon: props.location.marker_icon || 'Building2',
});
...
onMounted(async () => {
    ...
    if (typeof window !== 'undefined') {
        ...
        if (mapContainer.value) {
            mapInstance = L.map(mapContainer.value).setView([form.latitude, form.longitude], 8);
...
const submit = () => {
    form.put(`/admin/global-network/${props.location.id}`);
};
</script>
    ...
    <div class="space-y-6 max-w-5xl mx-auto">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <span class="font-mono text-xs uppercase tracking-widest text-amber">GLOBAL NETWORK EDIT</span>
                    <h1 class="text-2xl font-bold font-display text-slate-900">Edit Location: {{ location.name }}</h1>
                </div>
            </div>
            <button 
                @click="submit" 
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
                    <div class="flex justify-between items-center border-b pb-3">
                        ...
                        <span class="text-xs text-slate-500 font-mono">Click or drag marker to update lat/lng</span>
                    </div>
