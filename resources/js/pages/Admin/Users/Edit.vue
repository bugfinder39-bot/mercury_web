...
import { ArrowLeft, Save, Upload, User, Shield, Lock, Mail, CheckCircle2, X, Trash2 } from '@lucide/vue';
...
const avatarPreview = ref<string | null>(props.adminUser.avatar || null);
...
const form = useForm({
    ...
    role: props.adminUser.role,
    status: props.adminUser.status,
...
const submitForm = () => {
    form.post(`/admin/users/${props.adminUser.id}`, {
        forceFormData: true,
    });
};
</script>
...
<template>
    <Head :title="`Edit User: ${props.adminUser.name} - Mercury CMS`" />
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="font-body text-xs text-slate-500">
                        Update details, role, account status, or credentials for {{ props.adminUser.name }}.
                    </p>
                </div>
            </div>
        </div>
            ...
            <form @submit.prevent="submitForm" class="space-y-6">
                ...
                <div class="flex items-center gap-6 border-b pb-6" style="border-color: rgba(11, 37, 64, 0.08)">
                    <div class="relative">
                        ...
                        >
                            <img :src="avatarPreview" :alt="form.name" class="size-full object-cover" />
                        </div>
                        <div
                            v-else
                    ...
                    <div>
                        <h3 class="font-display text-sm font-semibold" style="color: #0B2540">Profile Photo</h3>
                        <p class="text-xs text-slate-500 mt-0.5">JPG, PNG, GIF, or WebP. Max file size 2MB.</p>
                        <div class="mt-2 flex items-center gap-2">
                            <label
                                class="inline-flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-1.5 font-body text-xs font-semibold transition-colors"
                            ...
                            <button
                                v-if="avatarPreview && !isAvatarRemoved"
                                type="button"
                                @click="removeAvatar"
                ...
                <!-- Optional Password Change Box -->
                <div
                    class="rounded-xl border p-4 space-y-4"
                    style="background-color: #F8FAFC; border-color: rgba(11, 37, 64, 0.1)"
                >
                    <div>
                        <h4 class="font-display text-xs font-bold uppercase tracking-wider" style="color: #0B2540">
                            Change Password (Optional)
                        </h4>
                        <p class="text-xs text-slate-500 mt-0.5">Leave password fields blank to keep current password.</p>
                    </div>
                        ...
                        <div>
                            ...
                            <div class="relative mt-1">
                                ...
                                <input
                                    ...
                                    class="w-full rounded-lg py-2.5 pl-10 pr-4 font-body text-sm transition-colors focus:outline-none"
                                    style="background-color: #FFFFFF; border: 1px solid rgba(11, 37, 64, 0.15); color: #0B2540"
                                />
                            </div>
